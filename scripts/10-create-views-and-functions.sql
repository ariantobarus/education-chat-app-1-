-- Create useful views and functions for the application (MySQL Compatible)
...
DROP FUNCTION IF EXISTS `calculate_user_overall_progress`$$
CREATE FUNCTION `calculate_user_overall_progress`(user_id_param BIGINT)
RETURNS DECIMAL(5,2)
DETERMINISTIC
BEGIN
    DECLARE overall_progress DECIMAL(5,2);
...
DROP FUNCTION IF EXISTS `get_user_best_score`$$
CREATE FUNCTION `get_user_best_score`(user_id_param BIGINT, test_type_param VARCHAR(10))
RETURNS INTEGER
DETERMINISTIC
BEGIN
    DECLARE best_score INTEGER;
...
CREATE FUNCTION `get_unread_message_count`(user_id_param BIGINT)
RETURNS INTEGER
DETERMINISTIC
BEGIN
